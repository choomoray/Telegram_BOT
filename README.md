...
**版本:** 0.5.37 | **运行环境:** Node.js | **数据库:** MongoDB Atlas
...
- 就绪后往**通知群话题**播报：每次启动都发「🚀 BOT已启动」；若本次是看门狗从崩溃中拉起来的，再补一条「♻️ BOT重启成功」（见 `utils/crashNotify.js`）
- **通知专用会话**（启动/崩溃话题群 + `IGNORED_CHAT_IDS`）在所有事件入口都被挡掉：不收录消息、不登记 `channel_group`、不处理成员变动
...
bot 每次要与**云端数据库 / Telegram API** 打交道都要等（Atlas 往返、`getChat` 补正会话类型、
缩略图现下），这期间**不能让用户端一片安静**（用户会以为 bot 卡了或网不好）。统一做法是
**"先给状态、再就地刷新结果"**（`utils/reply.js` 里的 `loadingMsg / processingMsg / withLoadingEdit`
是这套文案与流程的公共实现）：

| 场景 | 及时反馈 |
| ---- | -------- |
| 进入 `/send`（要查库取频道/群组列表 + 按 Telegram 真实类型逐个补正） | 先发「♻️ 正在获取频道 / 群组 列表，请稍候...」→ 取到后**就地 edit** 成选择面板（不残留两条消息）；取不到就 edit 成「❌ 获取频道 / 群组列表失败，请稍后重试」，不会一直停在"正在获取" |
| `/send` 翻页 / 选择目标 | 先 `answerCallbackQuery` 回执（按钮转圈立刻停）+ 面板换成「♻️ 正在获取列表 / 正在切换目标」→ 再刷新成结果（回执只答一次，答两次第二个必报 query 失效） |
| `/send` 发送文本 / 单个媒体 / 媒体组 | 先回「♻️ 正在发送并收录中，请耐心等待...」（单个媒体放在**去重查询之前**）→ 发送 + 收录完成后就地刷新成「✅ 已发送到 X」/ 错误提示 |
| `/message_reply`、`/tag` 定位目标媒体 | 先回「🔍 正在查询中 / 正在查找媒体组...」→ 定位结果就地刷新；就绪后切换回复位置时先回「♻️ 正在切换…」 |
| `/search`、`/clean`、`/log`、`/random_*`、`/media_group`、`/setting` | 各自先回「♻️ 查询中 / 空数据查找中 / 日志正在查询中 / 正在搜集图片中 / 正在拉取云端设置...」，完成或失败都刷新同一条消息 |
...
收到群组/频道消息
    │
    ├── 通知专用会话？(isIgnoredChat) → 忽略（不收录、不响应）
    ├── 只处理管理员发的：(频道帖 / 频道自动转发) 天然放行；否则 from.id 必须在 ADMIN_CHAT_ID 里
    │      非管理员 → 静默忽略（不进库、不提示）
    │
    ├── 提取媒体信息 (extractMediaFromMessage)
    ├── 频道帖：落库窗口登记 in-flight（让"自动转发副本"那条并发处理链知道频道侧在写库）
    ├── 频道转发识别 (resolveChannelForwardInfo)
    │   ├── 是（forward_origin / 自动转发的 sender_chat / 绑定库正反向）→ 不重复收录，
    │   │      先等频道侧落库（in-flight）再决定；记录 message.channel_forward
    │   │      （频道源 + 群组位置）+ media 双位置（group/channel）
    │   └── 否 → 继续
    │
...
| `/message_reply [N]`     | messageReply.js        | 消息回复         | 进入 messageReply 模式，**回复位置默认群组**（定位后直接就绪，不再先弹按钮让用户选一次；频道转发媒体等同时存在群组/频道位置时，就绪消息上带「🔄 更改为发送至📢 频道」一键切换按钮，随时可改）；回复位置取自 `message.channel_forward` **与 `media.group`/`media.channel` 两处**，空描述媒体（没有 message 记录）同样可定位回复；N>=2 时媒体按 N 个为一组打包为媒体组回复（满 N 个立即回复一组，不足 N 的余量等待补满下一组，退出/超时时才冲刷发出）。单条/整组回复成功后**自动进入打标签（不退出回复模式）**，可继续发媒体继续回复。**就绪后也可直接发文字回复**（保留 Telegram 文本格式，并按新的 subgroup 收录成 `media_type='text'`，见下）                                     |
...
>
> **重排 / 刷新都不动浏览位置（重要）**：容器高度一旦变小，浏览器会把滚动位置**夹回去**（超出就归零），
> 表现就是"滚到中间一看，缩略图（尤其视频，缩略图是现从 Telegram 下的、来得最晚）一刷新就跳回最上面"。
> 因此：① 量高度时**不再先把卡片复位成 `span 1`**（卡片 `align-self: start`，直接量就是内容高度；
> 复位会让容器总高度瞬间塌成 8px/张）；② 排布与整视图刷新前后都用
> `captureScrollPositions / restoreScrollPositions` 把滚动位置记下再放回（含窄屏时滚动的文档）。
> 只有**换视图**才回到顶部，同一视图的刷新（自动刷新 / `↻ 刷新` / `R` / 操作后重载）保持当前位置。
...
**所有指令只能在私聊里执行**：在群组 / 频道里，除「回复一条消息 + `/edit`」这一种快捷编辑外，
任何指令都不会执行（**静默忽略**，不回应、不进入模式）；`.env` 没列出的人（非管理员）在私聊里
发指令同样**静默忽略** —— 不再回「❌ 无权使用该指令」之类的提示。
下表中 **`/search`、`/exit`、`/random_videos`、`/random_pictures` 对白名单用户与普通用户都开放**
（普通用户 = 在库、未封禁、非白名单；结果发到用户自己的私聊）；其余命令仅管理员可用。
...
| `/random_videos [N]`         | 随机获取视频（N 1~10 直接发送视频媒体，N>=11 标题列表展示）**普通用户也可用**                                                                                                                                                                                                                                                                                                                             | commands/randomVideos.js                                               |
| `/random_pictures [N]`       | 随机获取图片（N 1~10 张）**普通用户也可用**                                                                                                                                                                                                                                                                                                                                                               | commands/randomPictures.js                                             |
| `/mark`                      | 标记模式（单选题：发媒体正常标记 / 「📝 仅记录」只记录，完成即退出）                                                                                                                                                                                                                                                                                                                     | modes/markMode.js                                                      |
| `/send`                      | 发送模式（选择群组/频道发送并收录；**进入时先回「♻️ 正在获取频道 / 群组 列表」再就地刷新成选择面板**（取列表要查库 + 按 Telegram 真实类型逐个补正，期间不让用户干等），翻页 / 选目标同样是"先回执 + 加载态 → 刷新为结果"；发送 / 收录期间显示"正在发送并收录中"，完成后就地刷新为结果；可打标签——打标签面板为两区版面：上区=已有标签（点击移除）、下区=标签库（置顶在前、点击添加）；媒体组**描述先行带上**（发送时先内联带在第一条上，任何副本/自动转发都带描述），发完再还原到原本的媒体位置并对该媒体打标签；发文本同样收录为 `media_type='text'`）                                                | modes/sendMode.js                                                      |
...
> **只处理管理员发送的内容（重要）：** 群 / 频道里的消息**只有 `.env` 的 `ADMIN_CHAT_ID` 列出的用户发的才会被处理/收录**
> （直接发的媒体、文本、回复已收录媒体组加补媒体……都算），其它成员的消息一律静默忽略（不进库、不提示）。
> 两个天然例外：**频道帖**没有发送者（只有频道管理员能发帖）→ 照常收录；
> **「频道 → 关联讨论群」的自动转发**（`is_automatic_forward`）是频道自己的内容，已在频道侧收录 → 照常补群组位置。
> 普通成员**手动转发**频道帖不算，会被忽略（否则谁都能把群组位置抢过去）。
>
> **通知专用会话**（`STARTUP_NOTIFY_CHAT_ID` 那个启动/崩溃话题群，以及 `IGNORED_CHAT_IDS`）
> 什么都不做：不收录、不响应消息、不登记 `channel_group`、不处理成员变动 —— 它只是通知出口。

| 功能                  | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 媒体自动收录          | **管理员**发的群组/频道媒体自动入库（去重），带文本的媒体同步写入 message                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 频道转发双位置        | 频道转发至群组的媒体（含 `is_automatic_forward` 自动转发识别）在 message 记录新增 `channel_forward`、media 记录写入 `group`/`channel` 双位置，回复时可选回复在频道或群组；**来源识别按可靠度三级**：① `forward_origin` / `forward_from_chat`；② **自动转发的 `sender_chat`（= 来源频道，新版 Bot API 的自动转发不带 forward_origin）**；③ 绑定库正向（群→频道）/ 反向（频道→群）。**同一批媒体会经两条并发处理链到达**（`/send` 发到频道 + Telegram 立刻自动转发到关联讨论群；频道帖 + 讨论群里的自动转发副本）→ 落库窗口内用 `utils/inflight.js` 登记（**频道帖也会登记**），另一条链**先等它落库再决定**，因此只补位置、**不另建"无描述、可清理"的影子媒体组**，也不会把同一条媒体重复收录 |
...
| 关键字查询            | **管理员**在群组中发送文本自动搜索（非管理员的文本直接忽略）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 指令                  | 群里**只有**「回复一条消息 + `/edit`」会执行；其余指令（`/send`、`/clean`、`/tag`…）在群/频道里一律静默忽略（必须在私聊执行），单独发 `/edit`（不带回复）也忽略                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 成员记录              | 加入/退出自动记录，可配置封禁策略（通知专用会话不记录）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 入群审批              | 关联频道的用户自动通过加群申请（通知专用会话不审批）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
管理员 (ADMIN_CHAT_IDS)
  ├── 所有私聊命令（指令只在私聊可用）
  ├── 群组/频道里只有他发的消息会被收录（其它成员的消息一律忽略）
  ├── 群组/频道里可「回复一条消息 + /edit」快捷编辑
  └── 管理面板
       │
白名单用户 (white: 1)
  ├── 私聊搜索（关键字文本查询 + /search）
  └── /exit + 随机看片
       │
普通用户 (state: 1)
  ├── 私聊搜索（**关键字文本查询 + `/search`**，与白名单同一份搜索能力）
  └── 随机看片：`/random_videos`（随机视频）、`/random_pictures`（随机图片）+ `/exit`
       │
被封禁用户 (state: 0)
  └── 无法加入任何管理群组
       │
...
> **私聊准入分层**（`handlers/messageHandlers.js` + `db/users.getUserAccessTier`）：
> 管理员 → 全部功能；白名单用户 / **普通用户（在库、未封禁、非白名单）** → **搜索（关键字文本查询 + `/search`）+ 随机看片 + `/exit`**；
> 其余指令（`/send`、`/clean`、`/tag`…）对普通用户静默忽略；被封禁 / 未授权用户仍然收不到任何功能（只回一条拒因提示）。
> 搜索结果的「直接查看 / 跳转查看 / 翻页」按钮只把媒体发到**点按钮的那个人自己的私聊**，不会往管理群发帖。
>
> **无权限指令静默忽略**：非管理员在私聊里发未授权指令时，bot **不再回「❌ 无权使用该指令」**，
> 只写一条 warn 日志后忽略；`/help` 里那些按钮（`exec_cmd:`）被非管理员点到时同样静默（只把按钮转圈停掉，不回文案）。
