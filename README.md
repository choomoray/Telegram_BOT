...
下表中 **`/random_videos`、`/random_pictures` 对普通用户也开放**（发到用户自己的私聊），
`/search`、`/exit` 对白名单用户开放；其余命令仅管理员可用。
...
白名单用户 (white: 1)
  ├── 私聊搜索
  └── 基础查询功能（/search、/exit）+ 随机看片
       │
普通用户 (state: 1)
  ├── 无任何管理 / 查询权限
  └── 私聊可用随机看片：/random_videos（随机视频）、/random_pictures（随机图片）
       │
被封禁用户 (state: 0)
  └── 无法加入任何管理群组
       │
...
> 管理员 → 全部功能；白名单用户 → `/search`、`/exit` + 随机看片；
> **普通用户（在库、未封禁、非白名单）→ 只放行 `/random_videos`、`/random_pictures`**（结果发到自己的私聊）；
> 其余指令对普通用户静默忽略；被封禁 / 未授权用户仍然收不到任何功能（只回一条拒因提示）。
>
