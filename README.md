...
7. **（可选，推荐）看门狗模式（崩溃自动重启 + 通知管理员）：**
   ...
   - **重启报告**：重启后健康稳定 `WATCHDOG_RESTART_REPORT_AFTER`（默认 30 秒）才发「♻️ BOT重启成功」（确认是真成功，不是"起来又立刻崩"）；达到重启上限则发「🛑 已停止自动重启，请人工介入」；
   - **报告只列问题**：崩溃与重启报告都**只取最近的 warn / erro 日志**（最多 `WATCHDOG_REPORT_LINES` 条，去掉时间戳与颜色码），形如：
     ```
     ⚠️ BOT出现意外崩溃，稍后尝试重启
