...
| 🗄 数据库 | **集合浏览与集合明细整合为一个视图**：上栏 = 集合选择 / 排序 / 插入 / AI 翻译 / 🔄 重新统计 + 库信息；中部为整库汇总卡（集合数 / 文档总数 / 存储占用 / 数据体积 / 索引占用）；下方为**各集合明细表**（文档数、数据体积、磁盘占用、索引占用、索引数，按体积排序，**点任意一行即切换下方浏览的集合**，当前集合行高亮）。套件不允许 `dbStats`/`collStats` 时自动降级为仅文档数并给出原因。再下方为**原始文档浏览**：单集合分页浏览 / 「全部数据库」跨集合概览、文档 JSON 就地修改与删除、插入模板 |
